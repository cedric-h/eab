...
extern void LoadFontDefault(void)
{
    ...
    if (isGpuReady) defaultFont.texture = RL_LoadTextureFromImage(imFont);
...
extern void UnloadFontDefault(void)
{
    ...
    if (isGpuReady) RL_UnloadTexture(defaultFont.texture);
...
RL_Font RL_LoadFontFromImage(RL_Image image, RL_Color key, int firstChar)
{
    ...
    if (isGpuReady) font.texture = RL_LoadTextureFromImage(fontClear); // Convert processed image to OpenGL texture
...
RL_Font RL_LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
    ...
    if (font.glyphs != NULL)
    {
        ...
        if (isGpuReady) font.texture = RL_LoadTextureFromImage(atlas);
...
void RL_UnloadFont(RL_Font font)
{
    ...
    if (font.texture.id != RL_GetFontDefault().texture.id)
    {
        ...
        if (isGpuReady) RL_UnloadTexture(font.texture);
        RL_FREE(font.recs);
...
#define SUPPORT_COMPRESSED_FONT_ATLAS
    ...
    byteCount += sprintf(txtData + byteCount, "    if (isGpuReady) font.texture = RL_LoadTextureFromImage(imFont);\n");
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
...
static RL_Font LoadBMFont(const char *fileName)
{
    ...
    if (isGpuReady) font.texture = RL_LoadTextureFromImage(fullFont);
